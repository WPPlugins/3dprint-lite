...
window.p3dlite_uploader = new plupload.Uploader({
	...
	init: {
		...
		FilesAdded: function(up, files) {
			...
			window.wp.hooks.doAction( 'p3dlite.filesAdded');
			if (p3dlite.filereader_supported) {
				if (jQuery.inArray(file_ext, p3dlite.files_to_convert)==-1) {
					...
					reader.onload = function(event) {
						...
						window.wp.hooks.doAction( 'p3dlite.fileRead');
...
p3dlite_uploader.bind('BeforeUpload', function (up, file) {
	...
	window.wp.hooks.doAction( 'p3dlite.beforeUpload');
	});
...
//		}
	...
	window.wp.hooks.doAction( 'p3dlite.fileUploaded');
});
...
function p3dliteBindSubmit() {
	jQuery( "form.variations_form" ).on( "submit", function(e) {
		...
		window.wp.hooks.doAction( 'p3dlite.productScreenshot');
		return true;
	})
}
...
function p3dliteViewerInit(model, mtl, ext, is_string) {
	...
	//3D Renderer
	p3dlite.renderer = Detector.webgl? new THREE.WebGLRenderer({ antialias: true, canvas: p3dlite_canvas, preserveDrawingBuffer: true }): new THREE.CanvasRenderer({canvas: p3dlite_canvas});
	...
	if (Detector.webgl) {
		...
		p3dlite.renderer.shadowMap.Type = THREE.PCFSoftShadowMap;
	}

	p3dlite.camera = new THREE.PerspectiveCamera( 35, p3dlite_canvas_width / p3dlite_canvas_height, 1, 1000 );
	p3dlite.camera.position.set( 0, 0, 0 );
	p3dlite.cameraTarget = new THREE.Vector3( 0, 0, 0 );

	p3dlite.scene = new THREE.Scene();
	//p3dlite.scene.fog = new THREE.Fog( 0x72645b, 1, 300 );

	//Group
	if (p3dlite.group) p3dlite.scene.remove(p3dlite.group);
	p3dlite.group = new THREE.Group();
	...
	p3dlite.controls = new THREE.OrbitControls( p3dlite.camera, p3dlite.renderer.domElement );
	...
	if (ext=='stl')
		p3dlite.loader = new THREE.STLLoader();
	else if (ext=='obj') {
		p3dlite.loader = new THREE.OBJLoader();
	}
...
function p3dliteModelOnLoad(object) {
	...
	if (Detector.webgl) {
		...
		plane = new THREE.Mesh(
			...
			new THREE.MeshPhongMaterial ( { color: parseInt(p3dlite.ground_color, 16), transparent:plane_transparent, opacity:plane_opacity, shininess: plane_shininess } ) 
		);
		...
		if (p3dlite.ground_mirror=='on') {
			...
			p3dlite.groundMirror = new THREE.Mirror( p3dlite.renderer, p3dlite.camera, { clipBias: 0.003, textureWidth: canvas_width, textureHeight: canvas_height, color: 0xaaaaaa } );
			var mirrorMesh = new THREE.Mesh( planeGeo, p3dlite.groundMirror.material );
	...
	if (p3dlite.show_grid=='on' && p3dlite.plane_color.length>0) {
		...
		for ( var i = - size; i <= size; i += step ) {
			grid_geometry.vertices.push( new THREE.Vector3( - size, p3dlite.boundingBox.min.z, i ) );
			grid_geometry.vertices.push( new THREE.Vector3(   size, p3dlite.boundingBox.min.z, i ) );
			grid_geometry.vertices.push( new THREE.Vector3( i, p3dlite.boundingBox.min.z, - size ) );
			grid_geometry.vertices.push( new THREE.Vector3( i, p3dlite.boundingBox.min.z,   size ) );
		
		}


		var grid_material = new THREE.LineBasicMaterial( { color: parseInt(p3dlite.plane_color, 16), opacity: 0.2 } );
...
function p3dliteCreateMaterial(model_shading) {
	...
	var color = new THREE.Color( p3dliteGetCurrentColor() );
	...
	if (Detector.webgl) {
		...
		var material = new THREE.MeshPhongMaterial( { color: color, specular: model_shininess.specular, shininess: model_shininess.shininess, transparent:true, opacity:model_transparency, wireframe:false, shading:shading } );
	}
	else {

		var material = new THREE.MeshLambertMaterial( { color: color, vertexColors: THREE.FaceColors, wireframe: false, overdraw:1, shading:THREE.FlatShading } );
	}
	return material;
}
...
function p3dliteCreateModel(object, geometry, material, shading) {
	...
	if (shading=='smooth' && Detector.webgl) {
		...
		geometry = smooth_geometry;
                p3dlite.model_mesh = new THREE.Mesh(geometry, material);
	}
...
function p3dliteBoxFitsBox (dim_x1, dim_y1, dim_z1, dim_x2, dim_y2, dim_z2) {
	...
	fits=window.wp.hooks.applyFilters('p3dlite.boxFitsBox', fits, dim_x1, dim_y1, dim_z1, dim_x2, dim_y2, dim_z2);
	return fits;
}
...
function p3dliteBoxFitsBoxXY (dim_x1, dim_y1, dim_x2, dim_y2) {
	...
	fits=window.wp.hooks.applyFilters('p3dlite.boxFitsBoxXY', fits, dim_x1, dim_y1, dim_x2, dim_y2);
	return fits;
}
...
function p3dliteShowError(message) {
	...
	window.wp.hooks.doAction( 'p3dlite.showError');
}
...
function p3dliteSelectFilament(obj) {
	...
	window.wp.hooks.doAction( 'p3dlite.selectFilament');
}
...
function p3dliteSelectCoating(obj) {
	...
	p3dliteGetStats();
	window.wp.hooks.doAction( 'p3dlite.selectCoating');
}
...
function p3dliteSelectUnit(obj) {
	...
	window.wp.hooks.doAction( 'p3dlite.selectUnit');
}
...
//			jQuery(this).css('visibility', 'visible');
	...
	window.wp.hooks.doAction( 'p3dlite.selectPrinter');
}
...
*/
	...
	printable=window.wp.hooks.applyFilters('p3dlite.checkPrintability', printable);
...
//an example hook
window.wp.hooks.addFilter( 'p3dlite.calculatePrintingCost', function  (total, product_info) {
...
function p3dliteGetStatsClientSide() {
	...
	product_info=window.wp.hooks.applyFilters('p3dlite.getStatsClientSide', product_info);
	return product_info;
}
...
//	jQuery('.p3dlite-stats').hide(); 
	...
	window.wp.hooks.doAction( 'p3dlite.getStats');
...
//	if (window.p3dlite_uploader.state==1 && !p3dlite.checking) p3dliteDisplayQuoteLoading(false);
	...
	window.wp.hooks.doAction( 'p3dlite.showResponse');
}
...
//	var radius = p3dlite.model_mesh.geometry.boundingSphere.radius;
	if (platform_shape=='circle') {
		...
		material = new THREE.LineBasicMaterial({ color: parseInt(p3dlite.printer_color, 16) });
		...
		p3dlite.model_printer_bed = new THREE.Line(geometry, material);
		...
		p3dlite.model_printer = new THREE.Line( geometry, material );
 	...
 	window.wp.hooks.doAction( 'p3dlite.drawPrinterBox');
}
...
function p3dliteInitScaleSlider() {
	window.wp.hooks.doAction( 'p3dlite.p3dliteInitScaleSlider_start');
	...
	p3dliteGetStats();
	window.wp.hooks.doAction( 'p3dlite.p3dliteInitScaleSlider_end');
}
...
function p3dliteUpdateDimensions (obj) {
	window.wp.hooks.doAction( 'p3dlite.p3dliteUpdateDimensions_start');
